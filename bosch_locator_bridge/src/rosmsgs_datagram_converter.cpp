  ...
  geometry_msgs::PoseStamped & lidar_odo_pose)
{
  ...
  binary_reader >> client_localization_pose.errorFlags >> client_localization_pose.infoFlags;
  ...
  size_t scan_num, float scan_time)
{
  ...
  writer << static_cast<double>(scan_time != 0.0f ? scan_time : msg.scan_time);
  // numBeams
  ...
  // minRange
  writer << static_cast<float>(std::max(0.f, msg.range_min));
  // maxRange
